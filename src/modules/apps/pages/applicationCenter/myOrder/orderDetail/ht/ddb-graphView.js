...
class GraphView {
  ...
  getNodeBysstUUid(uuid) {
    let node;
    if (uuid) {
      this.$graphView.dm().each(element => {
        if (element.a("sst_uuid") === uuid) {
          node = element;
        }
      });
    }
  ...
  setIconJudge(dmObj) {
    if (dmObj.a("elementType") === "node") {
      if (dmObj.a("proType")) {
        ...
        if (dmObj.a("proType") !== "3") {
          let sst_uuid = dmObj.a("sst_uuid");
          switch (dmObj.a("sst_canvas_status")) {
            case "RedX":
              this._addIconForNode(dmObj, "error");
              dmObj._image.comps[0].borderColor = "red";
              this.$vue.wsSend(
                "channel:app:sst_canvas_status:" + sst_uuid,
                "sst_canvas_status"
              );
              console.log(sst_uuid);
              break;
            case "GreenTick":
              this._addIconForNode(dmObj, "success");
              dmObj._image.comps[0].borderColor = "#52C41A";
              break;
            case "SnowFlake":
              this._addIconForNode(dmObj, "loading");
              dmObj._image.comps[0].borderColor = "#6BADFF";
              this.$vue.wsSend(
                "channel:app:sst_canvas_status:" + sst_uuid,
                "sst_canvas_status"
              );
            ...
            case "GrayDisplay":
              console.log(dmObj);
              dmObj._image.comps[0].borderColor = "#ccc";
              dmObj._image.comps[0]["opacity"] = ".5";
              this.$vue.wsSend(
                "channel:app:sst_canvas_status:" + sst_uuid,
                "sst_canvas_status"
              );
