...
class ToolbarList {
  constructor() {
    this.self = [
      ...
      //     bottomRight: '0'
      //   },
      //   icon: require('@/commons/images/toolbar_delete@2x.png'),
      //   selectBackground: btnSelectColor,
      //   toolTip: '删除',
      //   onClicked: (graphView, $vue) => {
...
function _saveGraphData(graphView, $vue, status) {
  let canvas_text = JSON.stringify(JSON.parse(graphView.dm().serialize()));
  let mainNodeArr = [];
  let canvas_node_order;
  graphView.each(el => {
    if (el.a('elementType') === 'node') {
      if (el.a("proType") === proTypeSet.main) {
        mainNodeArr.push(el)
      }
    }
  });
  mainNodeArr.sort(function(a, b) { return a.getPosition().x - b.getPosition().x});
  canvas_node_order = mainNodeArr.map(node => { return node.a('obj_uuid') }).join(',');
  if (typeof $vue.saveCanvasData === 'function') {
    let data = {
      canvas_text,
      canvas_node_order
    };
    $vue.saveCanvasData(data, status);
  }
}
