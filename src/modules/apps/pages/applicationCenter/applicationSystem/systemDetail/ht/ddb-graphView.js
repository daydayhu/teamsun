...
class GraphView {
  ...
  _addEventListener() {
    ...
    graphView.getView().addEventListener("drop", e => {
      ...
      //   node => node.id === nodeId
      // );
      // console.log(paletteNodeData);
      // if (paletteNodeData) {
      //   paletteNodeData._name = paletteNodeData.name;
      //   paletteNodeData._id = paletteNodeData.id;
      //   if (paletteNodeData.comType == "upload_img") {
      //     isImageDarg = true;
      //   }
      // }
    });
    ...
    if (paletteNodeData) {
      ...
      this.$vue.addNodeForCanvas(gNode).then(res => {
        if (res.code === 200) {
          ...
          //this._saveGraphData(graphView, "drag");
        }
      });
    }
  }
  ...
  _saveGraphData(graphView) {
    ...
    this.$vue.saveCanvasData(saveObj.canvas_json, "hide");
  }
  ...
  _createNode(data, style) {
    ...
    if (data.comType == "upload_img") {
      node.setWidth(84);
      node.setHeight(84);
      node.setName(data.name);
      node.setImage(require("@/commons/images/upload-big.jpg"));
      this.$vue.isUpload = true;
      this.$vue.editData = node;
    } else {
      node.s({
        label: data.name,
        "label.position": 31,
  ...
  _addIconForNode(node) {
    let iconName = this._getNodeIconName(node)
      ? this._getNodeIconName(node)
      : "defaultIcon";
  ...
  _getNodeIconName(node) {
    ...
    if (node && node.a("image")) {
      iconImage = ht.Default.getImage(
        `${node.a("name")}-${node.a("service_id")}`,
        node.a("image")
      );
    }
    return iconImage ? `${node.a("name")}-${node.a("service_id")}` : "";
  }
