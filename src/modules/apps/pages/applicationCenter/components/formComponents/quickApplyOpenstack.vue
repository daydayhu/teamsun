    ...
    <section>
        <el-collapse v-model="activeNames" @change="handleChange" style="padding:15px 20px;">
            <el-form label-width="30%" ref="form" :model="form" :rules="rules">
                <el-collapse-item name="1" v-if="((base_info.indexOf(phaseState)>-1) || phaseState==2 || phaseState==4)">
                    ...
                    <el-form-item label="OS主机名（hostname）" prop="hostname">
                            <el-input
                                ...
                                :disabled="(phaseState==2 || phaseState==4) && !isEditable"
                                @blur="holdData">
                            </el-input>
                ...
                <el-collapse-item name="2" v-if="((capacity_info.indexOf(phaseState)>-1) || phaseState==2 || phaseState==4)">
                    <div class="collapse-title" slot="title">
                        <span>配置信息</span>
                ...
                <el-collapse-item v-if="((net_info.indexOf(phaseState)>-1) || phaseState==4)" name="3">
                    <div class="collapse-title" slot="title">
                        <span>网络信息</span>
                    </div>
                    <networkCard
                ...
                <el-collapse-item v-if="((deploy_target.indexOf(phaseState)>-1) || phaseState==4)" name="4">
                    <div class="collapse-title" slot="title">
                        <span>部署目标</span>
                        <!-- v-if="((deploy_target.indexOf(phaseState)>-1) || phaseState==4)" -->
                        <!-- <el-button type="text" class="slot-btn">高级参数</el-button> -->
                    </div>
                    <el-form-item label="计算可用域" prop="availability_zone">
                            <el-select
                                ...
                                style="width:440px"
                                @change="holdData"
                                :disabled="phaseState == '4' && !isEditable">
                                <el-option
                    ...
                    <el-form-item label="金属裸机" prop="ironic_node_id">
                        <el-row>
                                ...
                                <el-button type="text" @click="portPolymerization">
                                    端口聚合
                                </el-button>
                        </el-row>
                        <b>计算模板：</b><span>{{(typeof(form.flavor_name) == "undefined" ? "" : form.flavor_name)+","}}</span>
        ...
        <el-dialog
            :visible.sync="portAble"
            :title="'端口聚合详情'"
            width="700px"
            @closed="closePortDialog">
            <portPolymerizationDialog
                :uuid="form.ironic_node_uuid">
            </portPolymerizationDialog>
         </el-dialog>
    </section>
</template>
...
import portPolymerizationDialog from "./compontents/portPolymerizationDialog"
...
export default {
        ...
        getIronicTicketData(){
            requestMethod.getIronicTicket("get",this.form.node_canvas_uuid).then(res=>{
                if(res.data.is_created) {
                    ...
                    this.form.availability_zone = Number(this.form.availability_zone) ? Number(this.form.availability_zone):""
                    this.osVersion = this.form.os_version.split("-")
                    this.ticket_obj_id = res.data.ironic_data.ticket_obj_id
        ...
        selectBaremetals(index) {
            this.baremetalsInfo.forEach(item=>{
                if(item.ironic_node_id === index) {
                    ...
                    this.form.flavor_name = item.flavor_name
    ...
    mounted() {
        ...
        this.net_info=thedata.net_info.map(String)
        this.deploy_target=thedata.deploy_target.map(String);
        this.capacity_info=thedata.capacity_info.map(String);
    }
}
</script>
