...
class GraphView {
          ...
          case "node":
            if (currentElement.a("proType")) {
              ...
              if (currentElement.a("proType") === proTypeSet.main) {
                graphView
                  .getSelectionModel()
                  .setSelection(currentElement.getParent());
              }
            }
            break;
          case "group":
  ...
  _restoreGraphStyle() {
    if (!this.$vue.isLive) {
      this.updateAllNodeUUidFromCanvas();
    }
    let proTypeSet = this.$vue.proTypeSet;
    let arr = [];
    this.$graphView.dm().each(data => {
      let dmObj = data;
      console.log(data._attrObject);
      //data._attrObject ? (data._attrObject.originid = dmObj.a("uuid")) : "";
      if (dmObj.a("elementType") === "node") {
        if (dmObj.a("proType")) {
          this._addIconForNode(dmObj);
          arr.push({
            base_service_id: dmObj.a("service_id"),
            node_canvas_uuid: dmObj.a("uuid")
          });
        }
      }
    });
    if (!this.$vue.isLive) {
      this.$vue.quick_group_services(arr).then(res => {
        if (res.code == 200) {
          this._saveGraphData(this.$graphView);
          this._saveGraphRelations();
        } else {
          this.$vue.$message(res.message);
        }
      });
    }
  }
