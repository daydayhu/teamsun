...
7.设置默认搜索的值（暂定只支持默认值valueType=1的输入框。valueType=3多选；valueType=4单选）
用法：  引用的组件中 :defaultSearch="defaultSearch"
      输入框定义变量    defaultSearch:{
                    isInput: true,
                    isSave: true,
                    isShuxing: false,
                    isShuxingzhi: true,
    ...
    <div class="thesearchContent" ref='thesearchContent' v-clickoutside="handleClose">
        <div class="searchBox" ref='searchBox' @click='toFocus'>
            <div class="selectBox" ref='xialakuang' style='display:none'>
                <div class="choicebox">
                    <div class="thetop" v-if="isAllchoice==true">
                    ...
                    <div class="thetop" v-if="isAllchoice==false">
                        <el-radio-group v-model="theRadio" @change="handleRadioChange">
                          <el-radio v-for="city in cities" :label="city" :key="city" style='width:100px;height:26px;line-height:26px'>{{city}}</el-radio>
                        </el-radio-group>
                    </div>
                    <div class="thebottom">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      dealData(val) {
        ...
        for (var i = 0; i < val.length; i++) {
          for (var j = 0; j < this.selectArr.length; j++) {
            ...
            } else if (this.selectArr[j].value == val[i].shuxing && (this.selectArr[j].valueType == 3 ||this.selectArr[j].valueType == 4)) {//select
              var a = val[i].shuxingzhi.split('|');
              var b = this.selectArr[j].options;
              var d = this.selectArr[j].optionsValue;
              var c = '';
      ...
      checkboxChange(val, index) {
        ...
        this.$nextTick(function () {
          /* if (val.valueType != 3) {
           ...
           //增加单选的功能
          if(val.valueType==3 || val.valueType==4){
            this.$refs.xialakuang.style.display = 'block';
            this.$refs.refbox[index].appendChild(this.$refs.xialakuang);
            this.isfocus=false;
            if(val.valueType==3){
              this.isAllchoice=true;
            }else if(val.valueType==4){
      ...
      cancelSelect() {
        this.isfocus=true;
        if(this.choiceArr[this.focusIndex].valueType==4){
          this.theRadio=null
        }
      ...
      handleRadioChange(val){
        this.choiceArr[this.clickIndex].shuxingzhi = this.theRadio;
